...
const actions = {
    ...
    getBook({commit}){
        request
          ...
          .end((err,res)=>{
              if(!err){
                commit({type:'getBook',tag:'novel',res: res.body.books})
              }
          })
        request
          ...
          .end((err,res)=>{
              if(!err){
                commit({type:'getBook',tag:'reality',res: res.body.books})
              }
          })
        request
          ...
          .end((err,res)=>{
              if(!err){
                commit({type:'getBook',tag:'travel',res: res.body.books})
              }
          })
    }
}
