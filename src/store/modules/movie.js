...
const actions = {
    getMovie({commit}){
        ...
        .end((err,res)=>{
            if(!err){
                commit({type:'getMovie',tag:'hotMovies',res:res.body.subjects})
            }
        })
        request
        ...
        .end((err,res)=>{
            if(!err){
                commit({type:'getMovie',tag:'newMovies',res:res.body.subjects})
            }
        })
        request
        ...
        .end((err,res)=>{
            if(!err){
                commit({type:'getMovie',tag:'topMovies',res:res.body.subjects})
            }
        })  
    }
}
